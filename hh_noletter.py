...
def send_req(item):
    check = item[4].post('https://hh.ru/applicant/vacancy_response/popup', data={"incomplete": False, "vacancy_id":int(item[0]), "resume_hash": f"{item[1]}","ignore_postponed": True, "_xsrf":f"{item[2]}", "letter": False, "lux": True, "withoutTest": "no", "hhtmFromLabel": "undefined", "hhtmSourceLabel": "undefined"})
...
if __name__ == "__main__":
    ...
    while True:
        ...
        send_dict = []
        for link in links:
            send_dict.append((link, resume_hash, xsrf_token, req))
        if links == []:
            break
