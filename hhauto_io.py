...
async def send_req(item)
    async with aiohttp.ClientSession() as session
        check = await session.post('httpshh.ruapplicantvacancy_responsepopup', data={incomplete False, vacancy_idint(item[0]), resume_hash f{item[1]}, ignore_postponed True, _xsrff{item[2]}, letter f{item[3]}, lux True, withoutTest no, hhtmFromLabel undefined, hhtmSourceLabel undefined})
...
async def main()
    ...
    headers = {Host hh.ru, User-Agent Mozilla5.0 (Macintosh; Intel Mac OS X 10_15_7) AppleWebKit605.1.15 (KHTML, like Gecko) Version16.1 Safari605.1.15, Cookie f{cookies}}
    ...
    Создание запроса для поиска вакансий httpshh.rusearchvacancytext=автоматизация+python&salary=&schedule=remote&ored_clusters=true&enable_snippets=true
    ...
    async with aiohttp.ClientSession(headers=headers) as session
        ...
        while True
            ...
            links = re.findall('httpshh.ruvacancy(d)', data_text, re.DOTALL)
            send_dict = []
            for link in links
